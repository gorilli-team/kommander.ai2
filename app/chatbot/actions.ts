...
import { connectToDatabase } from '@/backend/lib/mongodb'; // Assumes this path resolves
import openai from '@/backend/lib/openai'; // Assumes this path resolves
import { buildPromptServer, type ChatMessage } from '@/backend/lib/buildPromptServer'; // Assumes this path resolves
import type { Faq } from '@/backend/schemas/faq'; // Assumes this path resolves
...
export async function generateChatResponse(
  ...
  try {
    ...
    console.log(`[app/chatbot/actions.ts] generateChatResponse: Fetched metadata for ${uploadedFilesInfo.length} uploaded files from 'raw_files_meta'.`);
