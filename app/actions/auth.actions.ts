...
export async function registerUser(values: unknown): Promise<{ success?: string; error?: string; details?: any }> {
  ...
  console.log('[app/actions/auth.actions.ts] Registration fields validated for email:', email);

  try {
    console.log('[app/actions/auth.actions.ts] Attempting to connect to database for registration...');
    const { db } = await connectToDatabase();
    console.log('[app/actions/auth.actions.ts] Connected to database. Checking for existing user:', email);
    ...
    console.log('[app/actions/auth.actions.ts] Email available. Hashing password...');

    const hashedPassword = await bcrypt.hash(password, 10); // Salt rounds = 10
    console.log('[app/actions/auth.actions.ts] Password hashed.');
    ...
    if (result.insertedId) {
      console.log('[app/actions/auth.actions.ts] User created successfully with ID:', result.insertedId);
      return { success: 'Account created successfully! You can now log in.' };
    } else {
      console.error('[app/actions/auth.actions.ts] Failed to insert user into database, no insertedId returned.');
      return { error: 'Failed to create account. Please try again.' };
    }
  } catch (error) {
    console.error('[app/actions/auth.actions.ts] Registration error:', error);
