...
export interface Message {
  id: string;
  role: 'user' | 'assistant' | 'system'; // System messages for errors/info
  content: string;
  timestamp: Date;
}
...
export function useChat() {
  ...
  const sendMessage = useCallback(async (userMessageContent: string) => {
    ...
    // Prepare history for the AI (excluding system messages from client-side)
  ...
  }, [messages, toast]); // Make sure `addMessage` is stable or included if it changes
