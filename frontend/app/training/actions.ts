...
export async function createFaq(data: unknown) {
  ...
  if (!session?.user?.id) {
    console.error('[frontend/app/training/actions.ts] createFaq: User not authenticated.');
  ...
  if (!validatedClientFields.success) {
    console.error('[frontend/app/training/actions.ts] createFaq: Validation failed:', validatedClientFields.error.flatten().fieldErrors);
  ...
  console.log('[frontend/app/training/actions.ts] createFaq: Validated data for user:', userId, { question, answer });
  ...
  try {
    ...
    console.log('[frontend/app/training/actions.ts] createFaq: FAQ inserted with ID:', result.insertedId, 'for user:', userId);
    ...
    console.error('[frontend/app/training/actions.ts] createFaq: Database error for user', userId, ':', errorMessage);
...
export async function getFaqs(): Promise<FaqDisplayItem[]> {
  ...
  const session = await auth();
  if (!session?.user?.id) {
    console.error('[frontend/app/training/actions.ts] getFaqs: User not authenticated.');
    return []; // Return empty if not authenticated
  }
  const userId = session.user.id;
  console.log('[frontend/app/training/actions.ts] getFaqs: Fetching FAQs for user:', userId);

  try {
    const { db } = await connectToDatabase();
    const faqsFromDb = await db.collection('faqs').find({ userId: userId }).sort({ createdAt: -1 }).toArray();
    console.log(`[frontend/app/training/actions.ts] getFaqs: Fetched ${faqsFromDb.length} FAQs for user: ${userId}.`);
    ...
    console.error('[frontend/app/training/actions.ts] getFaqs: Error fetching FAQs for user', userId, ':', errorMessage);
    return [];
  }
}
...
export async function updateFaq(id: string, data: unknown) {
  ...
  const session = await auth();
  if (!session?.user?.id) {
    console.error('[frontend/app/training/actions.ts] updateFaq: User not authenticated.');
    return { error: 'User not authenticated. Please log in.' };
  }
  const userId = session.user.id;
  ...
  console.log('[frontend/app/training/actions.ts] updateFaq: Validated data for user:', userId, { question, answer });
  ...
  try {
    ...
    if (result.matchedCount === 0) {
      console.warn(`[frontend/app/training/actions.ts] updateFaq: FAQ not found or user ${userId} not authorized to update ID ${id}.`);
      return { error: 'FAQ not found or you are not authorized to update it.' };
    }

    console.log('[frontend/app/training/actions.ts] updateFaq: FAQ updated successfully for user:', userId);
    revalidatePath('/training');
    ...
    console.error(`[frontend/app/training/actions.ts] updateFaq: Database error for ID ${id}, user ${userId}:`, errorMessage);
...
export async function deleteFaq(id: string) {
  ...
  const session = await auth();
  if (!session?.user?.id) {
    console.error('[frontend/app/training/actions.ts] deleteFaq: User not authenticated.');
    return { error: 'User not authenticated. Please log in.' };
  }
  const userId = session.user.id;
  ...
  try {
    ...
    if (result.deletedCount === 0) {
      console.warn(`[frontend/app/training/actions.ts] deleteFaq: FAQ not found or user ${userId} not authorized to delete ID ${id}.`);
      return { error: 'FAQ not found or you are not authorized to delete it.' };
    }

    console.log('[frontend/app/training/actions.ts] deleteFaq: FAQ deleted successfully for user:', userId);
    revalidatePath('/training');
    ...
    console.error(`[frontend/app/training/actions.ts] deleteFaq: Database error for ID ${id}, user ${userId}:`, errorMessage);
...
export async function uploadFileAndProcess(formData: FormData): Promise<{ success?: string; error?: string; fileId?: string }> {
  ...
  const session = await auth();
  if (!session?.user?.id) {
    console.error('[frontend/app/training/actions.ts] uploadFileAndProcess: User not authenticated.');
    return { error: 'User not authenticated. Please log in.' };
  }
  const userId = session.user.id;
  ...
  console.log(`[frontend/app/training/actions.ts] uploadFileAndProcess (GridFS): File received for user ${userId}: ${file.name}, Type: ${file.type}, Size: ${file.size}`);

  if (file.size > MaxFileSize) {
    console.error(`[frontend/app/training/actions.ts] uploadFileAndProcess (GridFS): File size exceeds limit for user ${userId}: ${file.size}`);
    return { error: `File size exceeds ${MaxFileSize / (1024 * 1024)}MB limit.` };
  }

  if (!AcceptedFileTypes.includes(file.type)) {
    console.error(`[frontend/app/training/actions.ts] uploadFileAndProcess (GridFS): Invalid file type for user ${userId}: ${file.type}`);
  ...
  try {
    ...
    revalidatePath('/training');
    console.log(`[frontend/app/training/actions.ts] uploadFileAndProcess (GridFS): END - Success for user ${userId}.`);
    ...
    console.error(`[frontend/app/training/actions.ts] EXCEPTION in uploadFileAndProcess (GridFS) for user ${userId}, file ${file?.name || 'unknown file'}:`, error);
...
export async function getUploadedFiles(): Promise<DocumentDisplayItem[]> {
  ...
  const session = await auth();
  if (!session?.user?.id) {
    console.error('[frontend/app/training/actions.ts] getUploadedFiles: User not authenticated.');
    return [];
  }
  const userId = session.user.id;
  console.log('[frontend/app/training/actions.ts] getUploadedFiles (GridFS): Fetching files for user:', userId);
  ...
  try {
    ...
    const filesFromDb = await db.collection('raw_files_meta')
      ...
      .sort({ uploadedAt: -1 })
      .toArray();
    console.log(`[frontend/app/training/actions.ts] getUploadedFiles (GridFS): Fetched ${filesFromDb.length} file metadata records for user: ${userId}.`);
    ...
    console.error('[frontend/app/training/actions.ts] getUploadedFiles (GridFS): Error fetching files metadata for user', userId, ':', errorMessage);
    return [];
  }
}
...
export async function deleteDocument(id: string) { 
  ...
  const session = await auth();
  if (!session?.user?.id) {
    console.error('[frontend/app/training/actions.ts] deleteDocument: User not authenticated.');
    return { error: 'User not authenticated. Please log in.' };
  }
  const userId = session.user.id;
  ...
  try {
    ...
    if (!metaDoc) {
      console.warn(`[frontend/app/training/actions.ts] deleteDocument (GridFS): Metadata document not found for ID: ${id} and user: ${userId}`);
    ...
    console.log(`[frontend/app/training/actions.ts] deleteDocument (GridFS): Found GridFS file ID: ${gridFsFileIdToDelete} for metadata ID: ${id}, user: ${userId}`);
    ...
    console.log(`[frontend/app/training/actions.ts] deleteDocument (GridFS): Document metadata with ID ${id} deleted successfully for user ${userId}.`);
    revalidatePath('/training');
    ...
    console.error(`[frontend/app/training/actions.ts] deleteDocument (GridFS): Database/GridFS error for ID ${id}, user ${userId}:`, errorMessage);
...
export async function getFileContent(gridFsFileIdString: string, userId: string): Promise<Buffer | { error: string }> {
  console.log(`[frontend/app/training/actions.ts] getFileContent: Attempting to retrieve file with GridFS ID: ${gridFsFileIdString} for user: ${userId}`);
  ...
  try {
    ...
    if (!metaDoc) {
      console.warn(`[frontend/app/training/actions.ts] getFileContent: File with GridFS ID ${gridFsFileIdString} not found for user ${userId}, or user not authorized.`);
      return { error: 'File not found or you are not authorized to access it.' };
    }
    console.log(`[frontend/app/training/actions.ts] getFileContent: User ${userId} authorized for file ${metaDoc.fileName}. Proceeding with GridFS download.`);
    ...
    console.error(`[frontend/app/training/actions.ts] getFileContent: Exception for user ${userId}, file ID ${gridFsFileIdString}:`, errorMessage);
