...
if (GlobalWorkerOptions.workerSrc !== PDF_JS_WORKER_SRC) {
    ...
    console.log(`[frontend/app/chatbot/actions.ts] pdfjs-dist GlobalWorkerOptions.workerSrc configurato a CDN: ${PDF_JS_WORKER_SRC}`);
}
console.log(`[frontend/app/chatbot/actions.ts] pdfjs-dist version: ${pdfjsVersion}`);
...
async function extractTextFromFileBuffer(buffer: Buffer, fileType: string, fileName: string): Promise<string> {
  ...
  try {
    if (fileType === 'application/pdf') {
      ...
      console.log(`[frontend/app/chatbot/actions.ts] Documento PDF caricato per ${fileName} con ${pdfDoc.numPages} pagine.`);
      for (let i = 1; i <= pdfDoc.numPages; i++) {
        const page = await pdfDoc.getPage(i);
        const textContent = await page.getTextContent();
        rawText += textContent.items.map(item => ('str' in item ? item.str : '')).join(' ') + '\n';
      }
      console.log(`[frontend/app/chatbot/actions.ts] Estrazione testo PDF completata con successo per ${fileName}. Lunghezza: ${rawText.length}`);
    } else if (fileType === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
      const result = await mammoth.extractRawText({ buffer });
      rawText = result.value;
      console.log(`[frontend/app/chatbot/actions.ts] Estrazione testo DOCX completata con successo per ${fileName}. Lunghezza: ${rawText.length}`);
    } else if (fileType === 'text/plain') {
      rawText = buffer.toString('utf-8');
      console.log(`[frontend/app/chatbot/actions.ts] Estrazione testo TXT completata con successo per ${fileName}. Lunghezza: ${rawText.length}`);
    } else {
      console.warn(`[frontend/app/chatbot/actions.ts] Tipo file non supportato per estrazione testo: ${fileType} per ${fileName}`);
    ...
    console.error(`[frontend/app/chatbot/actions.ts] Errore durante l'estrazione del testo da ${fileName} (tipo: ${fileType}):`, error.message);
    console.error(`[frontend/app/chatbot/actions.ts] Stack trace errore estrazione:`, error.stack);
    ...
    if (fileType === 'application/pdf' && error.message && (error.message.toLowerCase().includes("cannot find module './pdf.worker.js'") || error.message.toLowerCase().includes("setting up fake worker failed") || error.message.toLowerCase().includes("libuuid"))) {
        ...
        console.warn(`[frontend/app/chatbot/actions.ts] Errore inizializzazione componente elaborazione PDF per ${fileName}: ${error.message}`);
    }
...
export async function generateChatResponse(
  ...
  try {
    ...
    console.log('[frontend/app/chatbot/actions.ts] generateChatResponse: Connesso al database.');
    ...
    console.log(`[frontend/app/chatbot/actions.ts] generateChatResponse: Recuperate ${faqs.length} FAQ.`);
    ...
    console.log(`[frontend/app/chatbot/actions.ts] generateChatResponse: Recuperati metadati per ${allUploadedFilesMeta.length} file caricati da 'raw_files_meta'.`);
    ...
    if (allUploadedFilesMeta.length > 0) {
      ...
      console.log(`[frontend/app/chatbot/actions.ts] Tentativo di elaborazione del file più recente: ${mostRecentFileMeta.fileName} (GridFS ID: ${mostRecentFileMeta.gridFsFileId.toString()})`);
      
      const fileBufferResult = await getFileContent(mostRecentFileMeta.gridFsFileId.toString());

      if ('error' in fileBufferResult) {
        console.error(`[frontend/app/chatbot/actions.ts] Errore nel recupero del contenuto per il file ${mostRecentFileMeta.fileName}: ${fileBufferResult.error}`);
        ...
        console.log(`[frontend/app/chatbot/actions.ts] Buffer del file ${mostRecentFileMeta.fileName} recuperato con successo. Dimensione: ${fileBufferResult.length}. Inizio estrazione testo.`);
        ...
        if (extractedTextContentForPrompt.length > MAX_TEXT_LENGTH) {
          ...
          console.log(`[frontend/app/chatbot/actions.ts] Testo estratto per ${mostRecentFileMeta.fileName} troncato a ${MAX_TEXT_LENGTH} caratteri.`);
        }
        console.log(`[frontend/app/chatbot/actions.ts] Testo estratto da ${mostRecentFileMeta.fileName} per il prompt. Lunghezza: ${extractedTextContentForPrompt.length}`);
        if (extractedTextContentForPrompt.startsWith("Errore durante l'estrazione") || extractedTextContentForPrompt.startsWith("Impossibile estrarre il testo") || extractedTextContentForPrompt.startsWith("Spiacenti, l'elaborazione dei file PDF") || extractedTextContentForPrompt.startsWith("Si è verificato un problema con l'inizializzazione")) {
            console.warn(`[frontend/app/chatbot/actions.ts] Estrazione testo per ${mostRecentFileMeta.fileName} ha restituito un messaggio di errore/avviso: ${extractedTextContentForPrompt}`);
        }
      }
    } else {
        console.log('[frontend/app/chatbot/actions.ts] Nessun file caricato trovato per fornire contesto aggiuntivo.');
    }

    const messages = buildPromptServer(userMessage, faqs, filesForPromptContext, extractedTextContentForPrompt, history);
    console.log('[frontend/app/chatbot/actions.ts] generateChatResponse: Prompt costruito per OpenAI.');
    ...
    console.log('[frontend/app/chatbot/actions.ts] generateChatResponse: Completamento OpenAI ricevuto.');

    const assistantResponse = completion.choices[0]?.message?.content;

    if (!assistantResponse) {
      console.error('[frontend/app/chatbot/actions.ts] generateChatResponse: AI non ha restituito una risposta.');
      return { error: 'AI non ha restituito una risposta.' };
    }

    console.log('[frontend/app/chatbot/actions.ts] generateChatResponse: Successo, restituzione risposta AI.');
    return { response: assistantResponse.trim() };

  } catch (error: any) {
    console.error('[frontend/app/chatbot/actions.ts] generateChatResponse: Errore durante la generazione della risposta della chat:', error);
    console.error('[frontend/app/chatbot/actions.ts] generateChatResponse: Nome errore:', error.name);
    console.error('[frontend/app/chatbot/actions.ts] generateChatResponse: Messaggio errore:', error.message);
    console.error('[frontend/app/chatbot/actions.ts] generateChatResponse: Stack errore:', error.stack);
